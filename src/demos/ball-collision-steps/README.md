...
Run `pnpm dev` and open [your practice demo](http://localhost:5173/ball-collision-practice). Change `count` and `radius` in `store.ts` to try different amounts and sizes. At the end of step 4, add Devtools with sliders and an FPS counter. Step 5 keeps those controls and moves the defaults to `Settings` in `traits.ts`. Balls can leave the screen until step 3.
